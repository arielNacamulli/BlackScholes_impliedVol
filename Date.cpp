...
Date::Date(const string &s, unsigned format) {
    switch(format) {
        ...
        case ddMMMyyyy_:
            day = stoi(s.substr(0, 2));
            month = str2month(s.substr(3, 3));
            std::cout << s.substr(7) << std::endl;
            year = stoi(s.substr(7));
            break;
...
double e30vs360(const Date &startDate, const Date &endDate) {
    ...
    double res = (endDate.getYear() - startDate.getYear()) + (endDate.getMonth() - startDate.getMonth())/12.0 + (int)(d2 - d1)/360.0 ;
    return res;
}
...
unsigned str2month(const string &s){
    std::map<string, unsigned> months
            {
                    { "Jan", 1 },
                    { "Feb", 2 },
                    { "Mar", 3 },
    ...
    if (it == months.cend()){
        std::cerr << "No such month" << std::endl;
        return 0;
    }
    return (*it).second;
}